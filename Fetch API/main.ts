...
$(document).ready(() => {
  ...
  .then(function(myJson) {
    ...
    myJson.list.forEach((element:City) => {
      let li = <HTMLLIElement>document.createElement('li') // make sure you're castin to the most logical type
      $cityList.append(li)
      ...
      let onCityClick = (evt : any) => {
        let coordsData = $(evt.currentTarget).data('coord');
        let coords = <Coords>JSON.parse(coordsData);
      }
